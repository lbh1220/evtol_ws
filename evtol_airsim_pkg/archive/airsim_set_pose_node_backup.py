...
if __name__ == '__main__':
    try:
        ...
        imu_pub = rospy.Publisher('/modified/imu', Imu, queue_size=10)
        # 创建压缩图像的发布者
        camera_pub = rospy.Publisher('/modified/camera/compressed', CompressedImage, queue_size=10)

        # 创建激光雷达点云的发布者
        lidar_pub = rospy.Publisher('/modified/lidar', PointCloud2, queue_size=10)

        rospy.Subscriber('/mavros/imu/data', Imu, imu_callback)
        # 创建压缩图像的订阅者
        rospy.Subscriber('/airsim_node/Drone1/Camera_front/Scene/compressed', CompressedImage, camera_callback)

        # 创建激光雷达点云的订阅者
        rospy.Subscriber('/airsim_node/Drone1/lidar/Lidar_Avia_front', PointCloud2, lidar_callback)
