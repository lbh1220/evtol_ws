...
if __name__ == "__main__":
    ...
    while(not rospy.is_shutdown()):
        ...
        rospy.logwarn("current position in gazebo: x = %s, y = %s, z = %s", 
        ...
        if(current_pose.pose.position.z > 10):
            ...
            # rospy.loginfo("finished transition")
                ...
                local_pos_pub.publish(pose)
                rospy.loginfo("current target: x = %s, y = %s, z = %s", 
                            pose.pose.position.x,
                            pose.pose.position.y,
                            pose.pose.position.z)
            else:
                if(current_pose_gazebo.pose.position.x<1000 and current_pose_gazebo.pose.position.x > 50):
                    pose.pose.position.x = current_pose.pose.position.x + 200
                    pose.pose.position.y = 0
                    ...
                    rospy.loginfo("current target: x = %s, y = %s, z = %s", 
                            ...
                            pose.pose.position.y,
                            pose.pose.position.z)
                elif(current_pose_gazebo.pose.position.x>1000):
                    if(current_pose.pose.position.z>25):
                        pose.pose.position.x = current_pose.pose.position.x + 200
                        pose.pose.position.y = 0
                        pose.pose.position.z = 20
                        local_pos_pub.publish(pose)
